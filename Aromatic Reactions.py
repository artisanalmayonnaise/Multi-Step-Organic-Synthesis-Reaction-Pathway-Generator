...
class AromaticReactions:
    ...
    - Nucleophilic aromatic substitution by addition to activated aryl halide
    - Nucleophilic aromatic substitution by formation of benzyne intermediate from unactivated aryl halide
    ...
    @staticmethod
    def benzene_fluorination(smile):
        smile = Chem.MolToSmiles(Chem.MolFromSmiles(smile))
        smile_products = [smile]
        smile1 = smile_products[0]
        
        while FunctionalGroups.benzene(smile1) == True:

            # Checks if benzene is unsubstituted
            m1 = Chem.MolFromSmiles(smile1)
            patt1 = Chem.MolFromSmarts("[cH1][cH][cH][cH][cH][cH1]")

            # Checks if benzene is monosubstituted
            m2 = Chem.MolFromSmiles(smile1)
            patt2 = Chem.MolFromSmarts("[c;!H1][cH][cH][cH][cH][cH1]")

            # Checks if benzene is ortho-disubstituted
            m3 = Chem.MolFromSmiles(smile1)
            patt3 = Chem.MolFromSmarts("[c;!H1][c;!H][cH][cH][cH][cH1]")

            # Checks if benzene is ortho-disubstituted
            m4 = Chem.MolFromSmiles(smile1)
            patt4 = Chem.MolFromSmarts("[c;!H1][cH][c;!H][cH][cH][cH1]")

            # Checks if benzene is ortho-disubstituted
            m5 = Chem.MolFromSmiles(smile1)
            patt5 = Chem.MolFromSmarts("[c;!H1][cH][cH][c;!H][cH][cH1]")
            
            # If the benzene is unsubstituted
            if m1.HasSubstructMatch(patt1) == True:
                rxn1 = AllChem.ReactionFromSmarts("[c:1][c:2].[F:3]>>[c:1]([F:3])[c:2]", useSmiles=True)
                ps1 = rxn1.RunReactants((Chem.MolFromSmiles(smile1), Chem.MolFromSmiles("[F]")))
                smile_products.append(Chem.MolToSmiles(ps1[0][0]))
                del(smile_products[0])
                smile1 = smile_products[0]

            # If the benzene is monosubstituted
            if m2.HasSubstructMatch(patt2) == True:

                # Generate the ring substituents
                tmp = Chem.ReplaceCore(Chem.MolFromSmiles("c1c(F)cccc1"), Chem.MolFromSmarts("c1ccccc1"))
                Chem.MolToSmiles(tmp)
                rs = Chem.GetMolFrags(tmp, asMols=True)

                fragment = Chem.MolToSmiles(rs[0])
                for j in fragment:
                    if j.isdigit():
                        fragment = fragment.replace(j, "100")

                # If the ring substituent is a meta-director
                r1 = 

                # If the ring substituent is an ortho-, para-director
